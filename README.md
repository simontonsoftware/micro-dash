...
- Small payload size. This full package is `6,647 bytes` minified, while a bundle containing the corresponding lodash functions is `42,554 bytes`. Size comparisons when importing individual functions can be even more dramatic. Those comparisons are provided in each function's [documentation](https://simontonsoftware.github.io/micro-dash/typedoc).
