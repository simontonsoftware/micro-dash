...
- Small payload size. This full package is `4,364 bytes` minified, while a bundle containing the corresponding lodash functions is `41,420 bytes`. Size comparisons when importing individual functions can be even more dramatic. Those comparisons are provided in each function's documentation.
